...
interface ModstitchExtension {
     ...
     *
     * By default, Modstitch looks for the following files in the specified order:
    ...
    /**
     * The path, relative to the root of the resulting JAR, where [accessWidener] will be copied.
     *
...
open class ModstitchExtensionImpl @Inject constructor(
    ...
    override val accessWidener = objects.fileProperty().convention(project.layout.file(project.provider {
        ...
        val directories = listOf(project.projectDir, project.rootProject.projectDir)
        directories.distinct().flatMap { d -> fileNames.map { d to it } }.firstNotNullOfOrNull {
